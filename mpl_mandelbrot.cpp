...
int main(int argc, char *argv[]) {
  ...
  if (size * local_size < global_size && rank == size - 1) {
    local_size = global_size - (size - 1) * local_size;
  }

  MandelbrotParams params = MandelbrotParams(
      global_size, 0, local_size * rank, global_size, local_size);
  ...
  if (rank == 0) {
    ...
    int delta_j_max = global_size - (size - 1) * local_size;
    uint8_t *buffer = new uint8_t[delta_i * delta_j_max];

    for (int iRank = 1; iRank < size; ++iRank) {

      comm_world.recv(*buffer, iRank, mpl::tag_t(iRank));
    ...
    comm_world.send(*mset.data, 0, mpl::tag_t(rank));
  }
